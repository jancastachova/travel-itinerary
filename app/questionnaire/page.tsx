...
export default function Home() {
  ...
  const activities = ['food','beaches','nighlife','culture','hiking','adventure','museums'];
    ...
    <div className='flex flex-row items-center justify-center min-h-screen'>
      ...
      {currentStep === 0 && (
        <div>
          ...
          <p>{days} days</p>
          <input type="range" min={3} max={10} value={days} onChange={(e) => setDays(Number(e.target.value))}/>
    ...
    {activities.map(a => (
      <div key={a}>
        <input type='checkbox' id={a} value={a} onChange={(e) => {
          if (e.target.checked) {
            setAnswers({ ...answers, preferredTags: [...answers.preferredTags, a] })
          } else {
            setAnswers({ ...answers, preferredTags: answers.preferredTags.filter(t=> t !== a) })
          }
        }} />
